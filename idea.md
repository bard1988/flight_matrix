...
| 16 | **Skyscanner as a booking link** — on a cell click, add "Book via Skyscanner" alongside the existing source link(s), same treatment as the current Kiwi/Google Flights links (a plain deep link to a search results page, not a per-fare ID Skyscanner doesn't expose). Needs Skyscanner's own URL-slug/city-id scheme worked out (likely IATA-code-based, unlike Kiwi's city-name slugs) and a `_deeplink`-style builder. | S–M | todo (2026-09-12) |
| 17 | **One-way trips** — currently every search is a round trip (`SearchRequest` always carries a return date/nights range). Needs a "trip type" toggle, a `SearchRequest` shape that tolerates no return leg, and each provider's `fill_matrix`/`discover`/`verify` to support a one-way query. Touches the matrix shape too — one-way has no "nights", so the grid becomes a single departure-date axis, not a departure × return one. | L | todo (2026-09-12) |
| 18 | **Google verify only extracts the outbound leg's times** — `_parse_all_itineraries` (`google_flights.py`) reads `flight[2]` as one flat segment list and treats it as the outbound; `stops_back` is hardcoded `None` and no return departs/arrives/duration is ever parsed, even though Google's own round-trip response almost certainly carries the return leg's data too. Today the "Flight times" block (board-source/Kiwi's `itinerary_details`, covers both legs) is the only source of return-leg times in the panel. Needs reverse-engineering Google's raw payload shape for the return leg specifically. | M | todo (2026-09-12) |
| 19 | **The Depart/Return-hour filter is Kiwi-only** — `depart_hours`/`return_hours` are read only by `providers/kiwi.py`; TravelpayoutsProvider ignores them entirely, and `_base_provider` doesn't special-case `has_search_filters`, so under ESTIMATE_FIRST a search WITH an hour filter still fills its initial (fast) pass from Travelpayouts, unfiltered -- only the later Kiwi upgrade pass actually applies it. Likely fix: have `_base_provider` skip the cheap source outright when `request.has_search_filters` is true, same as the existing cache-reuse bypass for filtered searches (`board.py`'s `cached = None if request.has_search_filters else ...`). | S–M | todo (2026-09-12) |
