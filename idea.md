...
| 18 | **Google verify only extracts the outbound leg's times** — a round-trip Google Flights response is shaped around picking an outbound flight first and never surfaces the paired return flight's own segments in that one response (same payload shape the `fast-flights` library's own parser reads). | M | **done** (2026-09-12: `_return_leg` in `google_flights.py` runs a second, one-way query -- destination → origin, on the return date -- best-effort after the outbound result already succeeded; `returns`/`returns_arrive`/`return_segments` flow through `/api/verify` and show as "Return departs/arrives/stops" in the panel. Not persisted in the verified cache yet -- no columns for them -- so a cached-fallback read of the same cell still lacks them; only a live check has them.) |
| 19 | **The Depart/Return-hour filter is Kiwi-only** — `depart_hours`/`return_hours` are read only by `providers/kiwi.py`; TravelpayoutsProvider ignores them entirely. | S–M | **done** (2026-09-12: `_base_provider` now falls through to the live provider whenever `request.has_search_filters` is set, same as the existing cache-reuse bypass for a filtered search.) |
