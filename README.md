...
data/
  make_ramp.py            derives the green sequential ramp
...
The board uses a **diverging price scale, applied per matrix: green = that destination's
cheapest → neutral → red = its dearest.** Each card's own cheapest cell gets a **yellow
ring**; the cheapest cell on the whole board gets a thicker ring plus a yellow card border.
Yellow is *reserved* — it never appears in the ramp — so the marker can never be read as a
price level. That is also why the ramp's midpoint is a neutral rather than yellow.
...
board"). Verified across three cards: green cut off at ₪1,600 on one and ₪4,300 on another.

### The arms are asymmetric on purpose

Green↔red is the worst possible pair for red-green colour blindness. Measured with
Machado-Oliveira-Fernandes at severity 1.0, a **symmetric** diverging ramp gives:

| mirrored pair | normal vision | protanopia | deuteranopia |
|---|---|---|---|
| cheapest vs dearest | 30.0 | 14.3 | **1.5** |
| second vs second-last | 26.6 | 12.1 | **1.3** |

A ΔE of 1.5 is indistinguishable — the cheapest and most expensive cells would look
*identical* to a deuteranopic reader despite a 30.0 difference in normal vision.

So the arms are given different lightness: the green arm runs dark (OKLCH L 0.40) and the
red arm light (L 0.70). That lifts the poles to 19.3 / 31.1 and the worst mirrored pair to
7.1, so the scale still reads when hue is lost — you can tell cheap from dear by lightness
alone. Every cell also carries its price as text, and the table view lists everything, which
is the real backstop.

Each ramp step ships its own ink colour (`--qi0`…`--qi6`, black or white by measured
contrast), because a diverging ramp has *both* ends dark and a single flip index cannot
work. Minimum label contrast is 4.8:1.

Regenerate with `py -3 data/emit_ramp_css.py`; re-check CVD with
`py -3 data/check_diverging_cvd.py`.

## Previous sequential ramp (superseded)

The heatmap is a **sequential one-hue green ramp** encoding cheapness: deepest green =
cheapest, receding toward the surface as price rises. Steps are generated by
`data/make_ramp.py` in OKLCH at a fixed green hue with chroma pushed to just inside the
sRGB gamut, because interpolating in sRGB desaturates the mid-steps to grey.

Both ramps pass lightness-monotonicity, adjacent-ΔL and single-hue checks in light and
dark. They intentionally fail the *ordinal* light-end contrast check: that rule is for
discrete tiers, whereas a continuous sequential ramp is allowed to recede toward the
surface at its low end. The relief is that every cell carries a visible price label and a
full table view exists, so the receding end never carries meaning by fill alone.
`--ramp-flip` is the measured step at which cell ink switches to the contrasting token
(5 in light, 4 in dark); every cell's label clears 4.8:1 against its fill.
